...
connect(_Grid,[],_Occupied,_Solution).
connect(Grid,[Link|ListLinksTail],Occupied,[Solution|NewSolution]) :-
    
    findValid(Grid,Link,Occupied,NewSolution,[]), 
    connect(Grid,ListLinksTail,Occupied,Solution).

findValid(_Grid,link(Nr,pos(StartRow,StartCol),pos(StartRow,StartCol)),_Occupied,connects(Nr,RevPath),Path):-
    reverse([pos(StartRow,StartCol)|Path],RevPath).

findValid(Grid,link(Nr,pos(StartRow,StartCol),pos(EndRow,EndCol)),Occupied,Solution,Path) :-
    transition(pos(StartRow,StartCol),pos(NewRow,NewCol)),
    is_valid_position(Grid,pos(NewRow,NewCol)),
    not(member(pos(NewRow,NewCol),Occupied)),

    findValid(Grid,
                link(Nr,pos(NewRow,NewCol),pos(EndRow,EndCol)),
                [pos(NewRow,NewCol)|Occupied],
                Solution,
                [pos(StartRow,StartCol)|Path]
            ).

%R,C -> R+1,C
transition(pos(StartRow,StartCol),pos(NewStartRow,StartCol)):-
...
%R,C -> R,C+1
transition(pos(StartRow,StartCol),pos(StartRow,NewStartCol)):-
    NewStartCol is StartCol + 1.
...
sort_links(Links,_Grid,SortedLinks) :-
