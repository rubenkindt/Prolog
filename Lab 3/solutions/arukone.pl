...
% The base case verifies whether each position is used exactly once.
connect(Grid,[],UsedPositions,[]) :-
    findall(Position,is_valid_position(Grid,Position),AllPositions),
    length(AllPositions,Length),
    list_to_set(UsedPositions,Set),
    length(Set,Length).
connect(Grid,[link(Label,Position1,Position2)|RestOfLinks],UsedPositions,[connects(Label,Path)|RestOfSolution]) :-
    find_path(Grid,Position1,Position2,UsedPositions,Path),
    append(Path,UsedPositions,MoreUsedPositions),
    connect(Grid,RestOfLinks,MoreUsedPositions,RestOfSolution).

% find_path/5
find_path(Grid,Position1,Position2,UsedPos,Path) :-
    find_path(Grid,Position1,Position2,UsedPos,[Position2],Path).

% find_path/6
% This predicate finds a path between a start and end location in reverse order starting from the end location.
find_path(Grid,Position1,Position2,_,PathSoFar,[Position1|PathSoFar]) :-
    is_valid_edge(Grid,Position1,Position2).
    
find_path(Grid,Position1,Position2,UsedPos,PathSoFar,Path) :-
    is_valid_edge(Grid,PosInBetween,Position2),
    \+ member(PosInBetween,UsedPos),
    \+ member(PosInBetween,PathSoFar),
    find_path(Grid,Position1,PosInBetween,UsedPos,[PosInBetween|PathSoFar],Path).
